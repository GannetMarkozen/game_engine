...
struct VkEngine {
	...
	auto init(this VkEngine& self, const char* title, const i32 window_width, const i32 window_height) -> void {
		static constexpr auto WINDOW_FLAGS = static_cast<SDL_WindowFlags>(SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE);
		self.window = SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, window_width, window_height, WINDOW_FLAGS);
		...
		// Init swapchain.
		self.init_swapchain(window_width, window_height);
	...
	auto init_descriptors() -> void {
		const DescriptorAllocatorGrowable::PoolSizeRatio sizes[] = {
			DescriptorAllocatorGrowable::PoolSizeRatio{
	...
	auto init_default_data() -> void {
		...
		// Write the buffer.
		new(material_constants.allocation->GetMappedData()) GltfMetallicRoughness::MaterialConstants{
			.color_factors{1.f, 1.f, 1.f, 1.f},
			.metal_roughness_factors{1.f, 0.5f, 0.f, 0.f},
		};

		deletion_queue.enqueue([this, material_constants] {
		...
		GltfMetallicRoughness::MaterialResources resources{
			...
			.metal_roughness_image = white_image,
			.metal_roughness_image_sampler = default_sampler_linear,
			.data_buffer = material_constants.buffer,
			.data_buffer_offset = 0,
		};

		default_data = metal_roughness_material.write_materials(device, MaterialPass::OPAQUE, resources, descriptor_allocator);
	}
	...
	GltfMetallicRoughness metal_roughness_material;
...
inline auto GltfMetallicRoughness::build_pipelines(res::VkEngine& engine) -> void {
	const VkShaderModule mesh_vert_shader = vkutils::load_shader_module(SHADER_PATH("mesh.vert"), engine.device);
	const VkShaderModule mesh_frag_shader = vkutils::load_shader_module(SHADER_PATH("mesh.frag"), engine.device);
	...
	const VkPipelineLayoutCreateInfo pipeline_layout_create_info{
		...
		.pSetLayouts = layouts,
		.pushConstantRangeCount = 1,
		.pPushConstantRanges = &draw_push_constant,
	};

	VkPipelineLayout new_layout;
	VK_VERIFY(vkCreatePipelineLayout(engine.device, &pipeline_layout_create_info, null, &new_layout));

	opaque_pipeline.layout = new_layout;
	translucent_pipeline.layout = new_layout;
	...
	pipeline_builder
		...
		.set_depth_attachment_format(engine.draw_image.format);

	opaque_pipeline.pipeline = pipeline_builder.build(engine.device);

	pipeline_builder
		.enable_blending_additive()
		.disable_depth_test();

	translucent_pipeline.pipeline = pipeline_builder.build(engine.device);

	// Shader modules no longer needed.
	vkDestroyShaderModule(engine.device, mesh_vert_shader, null);
	vkDestroyShaderModule(engine.device, mesh_frag_shader, null);
}

inline auto GltfMetallicRoughness::write_materials(VkDevice device, const MaterialPass pass, const MaterialResources& resources, DescriptorAllocatorGrowable& desc_allocator) -> MaterialInstance {
	...
	case MaterialPass::TRANSLUCENT:
		instance.pipeline = &translucent_pipeline;
		break;
	}

	instance.descriptor = desc_allocator.allocate(device, material_layout);

	writer.clear();
	writer
		.write_buffer(0, resources.data_buffer, sizeof(MaterialConstants), resources.data_buffer_offset, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER)
		.write_image(1, resources.color_image.image_view, resources.color_sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER)
		.write_image(2, resources.metal_roughness_image.image_view, resources.metal_roughness_image_sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER)
		.update_set(device, instance.descriptor);
